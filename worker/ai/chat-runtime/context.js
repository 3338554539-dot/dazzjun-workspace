import { listAIMemories } from "./memory.js";
import { dateInTimeZone, getTodayTasks } from "./todoSelectors.js";
const MAX_CONTEXT_LENGTH = 11_500;
const records = (value) => Array.isArray(value)
    ? value.filter((item) => Boolean(item) && typeof item === "object")
    : [];
const text = (value, maxLength = 180) => String(value ?? "").trim().slice(0, maxLength);
const number = (value) => Number.isFinite(Number(value)) ? Number(value) : 0;
const boolean = (value) => value === true;
function recent(items, keys, limit) {
    return [...items]
        .sort((a, b) => {
        const dateFor = (item) => keys.map((key) => text(item[key], 40)).find(Boolean) ?? "";
        return dateFor(b).localeCompare(dateFor(a));
    })
        .slice(0, limit);
}
export function buildAIContext(workspace, now = new Date(), memories = []) {
...
export async function loadAIContext({ db, userId, now }) {
    if (!userId)
    ...
    if (row?.payload) {
        try {
            ...
            throw Object.assign(new Error("工作台上下文暂时无法读取"), { status: 500 });
        }
    }
